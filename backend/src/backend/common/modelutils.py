...
def filter_keyword(queryset, q: str, parser: Callable[[str], Q]):
    if any(q.split("|")):
        return queryset.filter(
            functools.reduce(
                ...
                (
                    functools.reduce(
                        operator.and_,
                        (parser(and_word) for and_word in filter(bool, or_word.split())),
                    )
                    for or_word in filter(bool, q.split("|"))
                ),
            )
        )
    else:
...
class TimeStampModel(models.Model):
    created_at = models.DateTimeField(_("Create Time"), auto_now_add=True, db_index=True)
...
class PathModel(models.Model):
    ...
    path = models.CharField(
        ...
        max_length=150,
        blank=True,
        help_text=_("Physical storage path, such as '1.3.5'"),
        unique=True,
    )
    path_display = models.CharField(_("Path Display"), max_length=150, blank=True)
    level = models.PositiveSmallIntegerField(_("Level"), default=1, db_index=True)
    ...
    def update_path_display(self, update_descendants: bool = True):
        ...
        if not self.parent:
            ...
            ids = self.path.split(".")
            locations = objects.filter(pk__in=ids).exclude(pk=self.pk).order_by("level")
            self.path_display = "/".join([loc.name for loc in locations] + [self.name])
