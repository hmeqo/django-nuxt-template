import { isModel, type BaseModel } from '@hmeqo/easymodel'
  ...
  beforeRequest(method) {
    ...
    if (method.meta?.multipart || data instanceof FormData) {
      ...
      if (!(data instanceof FormData)) {
        ...
        if (isModel(data)) data = (data as BaseModel).toRepresentation()
    ...
    async onSuccess(response, method) {
      if (response.headers.get('Content-Type')?.includes('application/json')) {
        ...
        if (method.meta?.model) {
          const model = method.meta.model.init(data)
          if (method.type === 'PUT' || method.type === 'PATCH') Object.assign(method.data!, model)
          return model
        }
        return data
      }
